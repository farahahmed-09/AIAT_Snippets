...
class StorageManagementService:
    ...
    async def maintain_source_video_cache(self, max_keep: int = 3) -> Dict:
        ...
        try:
            logger.info(
                f"Starting source video cache maintenance (keep={max_keep})...")
            ...
            for session_id in to_delete_ids:
                ...
                logger.info(
                    f"Archived session {session_id}: freed {space:.2f}MB")
            ...
            logger.error(
                f"Error in maintain_source_video_cache: {str(e)}", exc_info=True)
            raise
    ...
    async def _delete_session_video_from_disk(self, session_id: int) -> float:
        ...
        input_video = os.path.join(self.input_dir, str(
            session_id), "downloaded_video.mp4")
        ...
        output_video = os.path.join(
            self.output_dir, str(session_id), "session_video.mp4")
    ...
    async def restore_deleted_session_video(self, session_id: int) -> str:
        ...
        try:
            logger.info(
                f"Checking video availability for session {session_id}...")
            ...
            if not session or not session.get('drive_link'):
                raise Exception(
                    f"Session {session_id} not found or has no drive_link")

            logger.info(
                f"Re-downloading video for session {session_id} from Google Drive...")
            ...
            logger.error(
                f"Failed to restore session video: {str(e)}", exc_info=True)
            raise
    ...
    async def restore_deleted_snippet_video(self, snippet_id: int) -> str:
        ...
        try:
            ...
            output_dir = os.path.join(
                self.output_dir, str(session_id), "snippets")
            if snippet.get('storage_link'):
                snippet_path = os.path.join(
                    output_dir, snippet['storage_link'])
            ...
            logger.error(
                f"Failed to restore snippet video: {str(e)}", exc_info=True)
            raise
    ...
    async def cleanup_ephemeral_snippets(self, max_age_hours: int = 1) -> Dict:
        ...
        try:
            logger.info(
                f"Starting ephemeral snippet cleanup (max_age={max_age_hours}h)...")
            ...
            for snippet in ephemeral:
                try:
                    ...
                    if storage_link:
                        snippet_path = os.path.join(
                            self.output_dir, str(
                                session_id), "snippets", storage_link
                        )
                        if os.path.exists(snippet_path):
                            ...
                            deleted_count += 1
                            logger.debug(
                                f"Deleted ephemeral snippet: {snippet_path}")
            ...
            logger.error(
                f"Error in cleanup_ephemeral_snippets: {str(e)}", exc_info=True)
            raise
    ...
    async def get_storage_usage(self) -> Dict:
        ...
        try:
            ...
            cached_sessions = len(
                [s for s in all_sessions if s.get('source_video_stored')])
            ...
            logger.error(
                f"Error getting storage usage: {str(e)}", exc_info=True)
            raise
