...
def generate_intros(source_dir, output_dir, config_data, temp_dir):
    ...
    video_files = [f for f in os.listdir(source_dir) if f.endswith(
        ('.mp4', '.avi', '.mov', '.mkv'))]
    ...
    for filename in video_files:
        ...
        try:
            ...
            make_text_image(title, 35, 'yellow',
                            temp_title, FONT_GILROY_REGULAR)
            make_text_image(video_name_text, 40, 'white',
                            temp_vname, FONT_GILROY_BOLD)
...
def concat_final_videos(intro_dir, stitched_dir, output_dir, temp_dir):
    ...
    intro_files = [f for f in os.listdir(
        intro_dir) if f.endswith(('.mp4', '.mov', '.avi'))]
    ...
    for filename in intro_files:
        ...
        print(
            f"   Metadata: Intro({v1['width']}x{v1['height']} {v1['fps']:.2f}fps) | Main({v2['width']}x{v2['height']} {v2['fps']:.2f}fps)")
        ...
        if os.path.exists(temp_normalized_intro):
            os.remove(temp_normalized_intro)
...
def run_ui_pipeline(raw_videos_dir, config_path, base_output_dir):
    ...
    stitch_backgrounds(raw_videos_dir, stitched_output_dir,
                       config_data, temp_dir)

    print("--- 3. Final Concatenation ---")
    concat_final_videos(intro_output_dir, stitched_output_dir,
