...
from src.app.core.llm_client import audio_translation
...
class TranscriptionService:
    @staticmethod
    def _transcribe_chunk(chunk_path: str, start_time: int) -> list:
        """Helper to transcribe a single chunk."""
        try:
            logger.debug(f"Transcribing chunk: {chunk_path}")
            with open(chunk_path, "rb") as audio_file:
                transcript_response = audio_translation(audio_file=audio_file)
    ...
    def generate_transcript_from_video(video_path: str, output_dir: str, model_name: str = "medium") -> str:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                future_to_index = {
                    executor.submit(TranscriptionService._transcribe_chunk, path, start): i
